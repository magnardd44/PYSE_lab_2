...
class ElasticDataCenter:
    def __init__(self, env, lambda_, mu, Qmin, energy_cost_model):
        ...
        self.k = 0
        self.k_previous = 0
        self.server_pool = simpy.Resource(env, capacity=self.n * RESOURCES_PER_SERVER)
            ...
            if Q > self.Qmin:
                ...
                if users_per_server < 5:
                    if self.server_level > 0:
                        ...
                        self.k = k
                        self.server_level -= (
                            1  # Reduce the number of available resources
                        )
                        self.active_users += 1
    ...
    def user3(self, k, Q):
        with self.server_pool.request() as req:
            ...
            print(
                f"active users={self.active_users} Server Pool Count: {self.server_level}, Q: {Q}"
            )
...
class DataCenter:
    ...
    def increase_servers(self):
        ...
        self.elastic_data_center.server_level += RESOURCES_PER_SERVER
    ...
    def decrease_servers(self):
        if self.servers.level > 0:
            ...
            self.elastic_data_center.server_level = max(
                self.elastic_data_center.server_level - RESOURCES_PER_SERVER,
                0,
            )
    ...
    def log_server_change(self, action):
        print(
            f"[Time {self.env.now:.2f}] Server {action}: Current servers = {self.elastic_data_center.server_level}, Energy Price = {self.energy_cost_model.elprice}"
        )

    def condition_increase(self):
        active_users = self.elastic_data_center.active_users
        threshold_up = 5
        condition_result = (
            self.elastic_data_center.server_capacity <= 10
            and (
                self.energy_cost_model.elprice == "low"
                or self.energy_cost_model.elprice == "medium"
            )
            and active_users >= threshold_up
        )

        print(
            f"Total users: {active_users}, servers: {self.elastic_data_center.server_capacity}, elprice: {self.energy_cost_model.elprice}, condition_result: {condition_result}"
        )
        return condition_result

    def condition_decrease(self):
        users_per_server = self.elastic_data_center.active_users
        condition_result = (
    ...
    def price_change(self):
        while True:
            ...
            for level, duration in self.durations.items():
                ...
                print(
                    f"[Time {self.env.now}] Energy price changed to {self.elprice} ({self.current_price} NOK/kWh)"
                )
                yield self.env.timeout(
                    duration
                )  # Holding the price for the defined duration
